...
if __name__ == '__main__':
    ...
    Resnet18_attention = True
    Resnet18_attention_layered = False
    ...
    elif Resnet18_attention == True:
        model = ModifiedResNetAttention()
    elif Resnet18_attention_layered == True:
    ...
    writer = SummaryWriter(f'runs/model1_cifar100_{time.strftime("%Y%m%d_%H%M%S")}')
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if i % 100 == 99:
                ...
                if Resnet18_attention_layered == True:
                    print(
                        f"Conv branch weight: {sigmoidfunc(model.fusion.gamma.item()):.3f} | Attention Branch Weight: {1 - sigmoidfunc(model.fusion.gamma.item()):.3f}")
        ...
        print(
            f"Epoch [{epoch + 1}] | Train Accuracy: {train_accuracy:.4f}% | Test Loss: {test_loss:.4f} | Test Accuracy: {test_accuracy:.4f}%  | Time: {epoch_time:.1f}s")
